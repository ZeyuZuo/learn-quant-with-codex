# learn-quant-with-codex 课程设计 v4
...
v4 的优化重点不是继续增加更多主题，而是把课程从“主题覆盖完整”推进到“学习体验可完成、代码产物可验收、回测结论可审查”。参考教程给本项目带来的核心判断是：
...
初学者最容易卡住的不是某个复杂公式，而是不知道一段量化代码到底在解决哪一类问题。v4 采用以下认知阶梯：
...
47 节课不应该全部写成同一种文章。v4 将课程分成 5 类 lesson，便于控制节奏。
...
v4 课程分为 10 个模块、47 节课、9 个 Mini Project 和 1 个 Capstone。模块数量不继续扩张，优化重点放在每个模块的产物质量和前后依赖。
