# learn-quant-with-codex 课程设计 v4.4
...
课程应该采用“边学边建”的方式。每一节课都必须留下一个可运行、可检查、可继续扩展的产物。

课程站还需要支持概念回查。初学者遇到 `signal`、`drawdown`、`Sharpe`、`Adjusted Close` 这类术语时，应能在课程页看到简短解释，并通过课程列表回到相关课程。

## 2. 参考教程与设计启发

本课程参考了几类成熟教程的编排方式，但不照搬它们的内容。v4.4 的重点是把参考教程里的教学机制转成本站的课程设计硬约束：每节课必须有明确问题、代码产物、图表观察、练习反馈、编程任务和可验收 Checkpoint；每个模块必须有一个能进入 Capstone 的交付物。

详细参考分析见 [tutorial-reference-analysis.md](./tutorial-reference-analysis.md)。单节课写作、误区设计、Lab 使用和编程任务规范见 [course-authoring-guide.md](./course-authoring-guide.md)。本文件只保留对课程结构有直接影响的设计决策。

v4.4 的优化重点不是继续增加更多主题，而是把课程从“主题覆盖完整”推进到“学习体验可完成、代码产物可验收、回测结论可审查、最终报告可辩护、单节课写作可复制”。参考教程给本项目带来的核心判断是：
...
| --- | --- | --- | --- |
| QuantConnect Learning Center | lesson -> task -> result -> hint/solution | 每节课设置编程任务、验收命令、Checkpoint 和下一步产物 | 不进入云端实盘部署和券商连接 |
...
- 每节课都设计一个 “编程任务”，包含输入、约束、验收标准和测试要求。
...
- 每节课都有 “Explain -> Implement -> Test -> Reflect” 的可验收学习工作流。
- 编程任务不只是“帮我写代码”，而是包含上下文、约束、验收和反思问题。
- Checkpoint 不只总结知识，还要求用户确认自己生成了哪个代码或报告产物。

### 2.6 v4.4 课程优化结论

参考这些教程后，v4.4 课程做出 13 个关键选择：
...
5. 每节课可验收：每节课都要有 Quiz、动手练习、编程任务和 Checkpoint。
...
9. 进度必须可见：首页、课程目录和课程页都要显示“下一步产物”，避免用户只阅读不交付。
...
13. 写作矩阵化：48 节课都要有明确的误区、产物和反馈形式，详见 `course-authoring-guide.md`。

### 2.6.1 v4.4 相比 v4.3 的新增优化

v4.3 已经确定 10 模块、47 节课、9 个 Mini Project 和 1 个 Capstone，并把参考教程机制写进课程设计。v4.4.1 在不增加模块数量的前提下，把 Module 4 的 `signal` 和 `position` 拆成两节，因此实际课程为 48 节。这个调整来自 Zipline 和 QuantStart 对执行时点、订单延迟和 look-ahead bias 的强调：初学者必须先理解“信号只是想法”，再理解“仓位才承担收益”。v4.4.1 解决课程真正写作和学习体验中最容易失控的六个问题：

| 问题 | v4.4 处理方式 |
| --- | --- |
| 课程大纲完整，但单节课写作可能退回普通文章 | 新增 [course-authoring-guide.md](./course-authoring-guide.md)，规定每节课的 Question、Trap、Tiny Example、Python Artifact、Chart、Quiz、编程任务、Checkpoint |
| 参考教程启发停留在原则层 | 把 QuantConnect、ML4T、Backtrader、Zipline、QuantStart、EDHEC/Coursera 的机制转成 48 节课写作矩阵 |
| Lab 和课程页可能脱节 | 规定 Lab 必须回答具体观察问题，课程页链接 Lab 时必须说明“去实验页看什么” |
| 课程仍可能按模块横向割裂 | 新增纵向能力线，让数据、指标、执行、验证、报告能力在多个模块中反复出现 |
| 模块完成标准不够具体 | 新增模块进入 / 退出闸门，明确每个模块发布前必须留下的文件、图表、测试和报告片段 |
| 参考教程机制还不够可操作 | 新增“教程机制到学习动作”表，把 task、project、lab、diagnostic、report 转成每节课的可见动作 |

### 2.6.2 v4.4 参考教程到学习动作的转换

v4.4 把外部教程的启发进一步落到用户在页面上实际完成的动作。每节课不能只写“参考了某教程”，而要能指出用户会做哪一步。

| 参考教程机制 | 用户在本站完成的学习动作 | 课程页必须出现的证据 |
| --- | --- | --- |
| QuantConnect 的 task / result / hint | 先尝试 Quiz 或小练习，再复制编程任务，最后按命令验收 | 任务说明有验收命令，Checkpoint 写明成功标准 |
| ML4T 的项目作业和报告 | 每个模块把一个产物交给下一模块，最终汇入 Capstone | Mini Project 显示交付物、相关课程、运行命令和反思问题 |
| EDHEC / Coursera 的 Python lab | 公式、手算、Python 函数和图表在同一节课互相印证 | 课程页包含 Tiny Example、Python Artifact 和 Chart Observation |
| Backtrader 的逐步搭建 | 回测器从最小流程逐步增加参数、结果对象、报告和测试 | Module 5 不能一次性给完整系统，必须展示 v1 -> result -> report -> tests |
| Zipline 的执行现实 | 策略前先区分 signal、position、lag、cost 和执行简化 | Module 4 必须早于 Module 5 和 Module 6 完成，且 Signal 和 Position 分成相邻两课 |
| QuantStart 的偏差诊断 | 用户看到错误实现也能产出好图，再学会拆穿它 | Diagnostic Lesson 必须展示错误 / 正确对比和误读提醒 |
...
| --- | --- | --- | --- |
| lesson -> task -> result | QuantConnect Learning Center | 每节课的 Quiz、编程任务、Checkpoint | 课程页能说明任务是否完成，任务说明包含验收命令 |
| 项目作业驱动 | Georgia Tech ML4T | 9 个 Mini Project 和 1 个 Capstone | 课程内展示交付物、命令和反思问题 |
...
Mini Project 需要在课程内容和模块闸门里展示，包含模块、交付物、验收项、建议命令和相关课程入口。这样用户可以按项目推进，也可以从课程目录按知识点推进。
...
初学者最容易卡住的不是某个复杂公式，而是不知道一段量化代码到底在解决哪一类问题。v4.4 采用以下认知阶梯：

| 阶段 | 用户心智问题 | 对应模块 | 设计理由 |
| --- | --- | --- | --- |
| 边界 | 这是不是投资建议？我应该怎么学习和验收？ | Module 0 | 先避免错误期待，再开始写代码 |
...
4. Reflect：写下限制、误区和下一条编程任务。

这个节奏来自交互式编程教程和 QuantConnect task/result 的反馈设计，适合把任务说明写成可验收的小型工程工单，而不是只复制代码。
...
48 节课不应该全部写成同一种文章。v4.4 将课程分成 5 类 lesson，便于控制节奏。
...
- Module 4 必须把 `signal` 和 `position` 拆开：4.1 只讲信号是想法，4.2 再讲持仓滞后和 look-ahead bias。这样用户不会在同一页里把“规则输出”和“收益暴露”混成一个概念。
...
成熟教程的共同点不是“每章讲一个新主题”，而是让同一种能力在不同情境中重复训练。v4.4 用 6 条纵向能力线贯穿 10 个模块，避免课程横向割裂。

| 能力线 | 第一次出现 | 反复训练位置 | Capstone 中的证明 |
| --- | --- | --- | --- |
| 数据审查 | Module 1 的 OHLCV 和复权价格 | Module 7 多资产对齐、Module 9 数据偏差 | 报告能说明数据范围、复权列、缺失和限制 |
| 收益路径 | Module 2 的收益率和净值 | Module 5 buy and hold、Module 6 策略、Module 8 样本外 | 报告能展示价格、收益、净值和回撤，不只给最终收益 |
| 风险解释 | Module 3 的波动、回撤、夏普 | Module 5 报告、Module 6 策略对比、Module 7 组合 | 报告能解释至少 3 个指标盲点 |
| 执行假设 | Module 4 的 signal、position、lag、cost | Module 5 回测器、Module 6 策略、Module 8 参数实验 | 报告能说明仓位滞后、成本假设和简化边界 |
| 验证能力 | Module 4 的故意错误回测 | Module 8 参数扫描、随机策略、样本外 | 报告能说明为什么历史最优不代表未来 |
| 研究表达 | Module 0 的边界声明 | Module 5 报告、Module 8 验证报告、Module 9 Capstone | 报告语言不包含投资建议或收益承诺 |

课程页写作时必须至少连接一条能力线。比如“Close vs Adjusted Close”不是孤立数据知识，它连接到 Capstone 的数据审查；“交易成本”不是孤立费用概念，它连接到执行假设和策略公平比较。

### 3.16 模块进入 / 退出闸门

每个模块都应该像 ML4T 项目作业一样有明确完成标准。用户进入下一模块前，至少应该完成下面的退出闸门。

| 模块 | 进入闸门 | 退出闸门 |
| --- | --- | --- |
| Module 0 | 能运行项目基本命令 | 写下非投资建议边界，完成第一条带验收的编程任务|
| Module 1 | 理解本项目只用本地样例数据 | 生成数据质量报告，能解释 OHLCV、交易日缺口和复权列 |
| Module 2 | 有一条可信价格序列 | 实现收益率、复利、年化和净值函数，并能手算小例子 |
| Module 3 | 能得到收益路径 | 生成绩效指标表，能指出最终收益、夏普和胜率的局限 |
| Module 4 | 能读懂指标表 | 实现 signal -> position、turnover、cost，并能识别 look-ahead 示例 |
| Module 5 | 有仓位和成本函数 | 运行 buy and hold 回测，产出含回撤、指标、风险声明的报告 |
| Module 6 | 有统一回测器 | 用同一数据、lag、成本和指标公平比较三类策略 |
| Module 7 | 会做单资产策略对比 | 对齐多资产数据，生成等权或轮动组合报告 |
| Module 8 | 有策略和组合实验结果 | 展示参数扫描、样本内 / 外、随机策略或年度稳定性诊断 |
| Module 9 | 已有模块材料 | 交付完整 Capstone 报告，并通过 Python 报告验证函数 |

如果某模块页面已经写完但退出闸门无法验收，应优先补课程产物、命令或测试，而不是继续增加新课。

## 4. 整体课程地图

v4.4.1 课程分为 10 个模块、48 节课、9 个 Mini Project 和 1 个 Capstone。模块数量不继续扩张，优化重点放在每个模块的产物质量、前后依赖和单节课可执行性。第 48 节来自 Module 4 的教学拆分：`signal` 和 `position` 是两个连续但不同的学习动作。

| 模块 | 主题 | 核心问题 | 参考教程启发 | 模块产物 |
| --- | --- | --- | --- | --- |
| 0 | 学习方式和项目边界 | 这门课如何学，哪些事情不能做？ | 交互式编程教程的任务边界 | 学习计划和第一条编程任务|
...
| --- | --- | --- | --- | --- |
| Module 0 | 会打开网站和运行基本命令 | 课程边界、可验收任务、测试驱动起点 | `learning-notes.md` 和第一个测试函数 | 能说清非投资建议边界，能写带验收的任务说明 |
...
用户最初提出的主题必须全部落到课程主线中。实现时不应另开一套旧大纲，而应按下表归入 v4.4 模块。
...
| 1.2 | 一行日线数据 | 日期索引、交易日缺口、日线粒度 | `DatetimeIndex` 处理 | 日历和价格点图 | 检查日期排序 | 能说明周末为什么没有日线 |
...
- 生成 `sample_prices_quality_report.json`。
...
| 4.5 | 一个故意错误的回测 | 错误 shift 的后果 | bias demo 函数 | 错误/正确净值对比图 | 审查 look-ahead bias | 能发现一个常见回测错误 |
...
| 9.2 | 为什么回测不能代表未来 | 不确定性、市场变化、流动性、执行 | 风险说明模板 | 回测结果改写练习 | 把夸张结论改为严谨结论 | 能写出合格风险声明 |
...
每节课都应有一个小型动手练习面板，目标是让用户在进入编程任务之前先完成最小理解闭环。
...
### 10.1.1 Mini Project 模板

课程内的每个 Mini Project 必须采用统一模板，避免项目作业变成一句描述。
...
完成 48 节课、9 个 Mini Project 和 Capstone。
