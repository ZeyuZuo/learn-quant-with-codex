# learn-quant-with-codex 课程设计 v4.1
...
本课程参考了几类成熟教程的编排方式，但不照搬它们的内容。v4.1 的重点是把参考教程里的教学机制转成本站的可执行规则：每节课必须有明确问题、代码产物、图表观察、练习反馈、Codex 任务和可验收 Checkpoint。

详细参考分析见 [tutorial-reference-analysis.md](./tutorial-reference-analysis.md)。本文件只保留对课程结构有直接影响的设计决策。

v4.1 的优化重点不是继续增加更多主题，而是把课程从“主题覆盖完整”推进到“学习体验可完成、代码产物可验收、回测结论可审查”。参考教程给本项目带来的核心判断是：
...
初学者最容易卡住的不是某个复杂公式，而是不知道一段量化代码到底在解决哪一类问题。v4.1 采用以下认知阶梯：
...
47 节课不应该全部写成同一种文章。v4.1 将课程分成 5 类 lesson，便于控制节奏。
...
v4.1 课程分为 10 个模块、47 节课、9 个 Mini Project 和 1 个 Capstone。模块数量不继续扩张，优化重点放在每个模块的产物质量和前后依赖。
...
用户最初提出的主题必须全部落到课程主线中。实现时不应另开一套旧大纲，而应按下表归入 v4.1 模块。
