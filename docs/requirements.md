...
`learn-quant-with-codex` 是一个面向初学者的交互式课程型 Web 项目，主题是“学习美股量化交易基础”。项目名里的 Codex 表示项目由 Codex 辅助制作和维护，不表示课程主题是学习 Codex。

项目同时包含两部分：

1. 一个现代课程站：通过课程、图表、Quiz、代码示例、编程任务和 Checkpoint 帮助用户循序渐进学习。
...
- 课程内 Mini Project 和最终报告材料
  - Mini Project 不再作为独立页面，而是融合在课程和模块闸门里
  - 最终报告要求由第 9.4 课和 Python 报告脚本承接
  - 复盘模板由课程页提供，学习者保存到自己的本地笔记或报告草稿中
...
需求文档只规定产品层面的课程约束；详细课表、模块产物、Mini Project、Capstone、编程任务模板和课程与代码同步契约，以 `docs/course-design.md` 的 v4.4.1 设计为准。课程内容的实际写作必须同时满足 `docs/course-authoring-guide.md` 的单节课结构、Lesson Brief、误区矩阵、纵向能力线、Lab 使用规则和作者自查清单。
...
- 每节可验收：课程页必须包含 Quiz、编程任务、Checkpoint 和至少一个可保留的学习产物。
...
- 课程页应显示本节对应的 Python 产物、常见误区、图表观察问题、任务验收命令和 Checkpoint。
- 课程页应能说明本节材料如何进入 Mini Project 或最终报告。
- 模块闸门应在课程内容中展示交付物、运行命令、验收项、反思问题和下一模块用途。
- 第 9.4 课和 Python 报告脚本应按数据、收益、风险、执行、验证、研究表达六条能力线检查最终报告。
...
| 参考机制 | 产品约束 |
| --- | --- |
| QuantConnect lesson/task/result | 每节课必须有可复制编程任务、验收命令或可检查产物 |
...
- Mini Project 融合在课程和模块闸门里，每个项目都有交付物、验收项、建议命令、相关课程入口和反思问题。
- Mini Project 的输出会逐步成为 Capstone 材料，例如数据质量报告、绩效指标表、策略对比、参数扫描和样本外验证。
- 最终报告要求由第 9.4 课和 Python 报告脚本承接，必须包含数据范围、策略假设、基准比较、成本处理、参数验证、样本外结果、偏差检查和非投资建议声明。
...
- 每节课提供可复制的复盘模板。
- 复盘模板覆盖概念理解、代码证据、图表观察、误区和 Checkpoint。
- 学习者把复盘保存到自己的本地笔记或报告草稿中。
- 笔记和复盘只服务学习反思，不保存交易账户、投资记录或实盘数据。
...
- 用户能从首页理解项目目标和学习路径。
- 用户能进入课程目录并按顺序学习。
- 每节课包含目标、概念、公式或直觉、代码、图表、练习、编程任务和 Checkpoint。
- 网站明确声明不提供投资建议。
