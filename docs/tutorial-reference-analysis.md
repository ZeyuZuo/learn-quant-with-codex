...
5. 编程任务必须像小型工程工单。任务说明必须写清背景、目标、约束、验收和反思。
...
| 来源 | 主要观察 | 对本项目的启发 |
| --- | --- | --- |
| QuantConnect Learning Center / Boot Camp | 课程拆成 lesson、task、result、hint、solution，强调完成小任务后获得反馈 | 每节课必须有可验收的编程任务，而不只是阅读材料 |
...
1. QuantConnect 的 lesson 不是普通文章，而是 task、result、hint / solution 组成的反馈系统。本站不能只给任务说明，还要在课程页显示验收命令、失败后应检查什么、完成后留下什么。
...
- 每节课都不只放文字，而是给一个可复制的编程任务。
- 编程任务必须有验收命令，例如 `uv run pytest` 或生成报告脚本。
- 课程页面需要显示 Checkpoint，告诉用户本节应该保留哪个产物。
- 我们不用 QuantConnect 的云端 IDE 和实盘能力；编程任务只面向本地学习项目。
...
| --- | --- | --- | --- |
| QuantConnect 的 task/result | 每节课有可复制编程任务和验收命令 | PromptBox、Checkpoint、完成状态 | 测试命令可实际运行 |
| ML4T 的项目作业 | 每个模块有 Mini Project，Capstone 有报告 Rubric | 课程内容展示验收清单 | examples 脚本生成报告材料 |
...
| 编程任务| 交互式编程工作流 | 任务说明必须有背景、目标、约束、验收、反思 |
...
| 阶段 | 用户问题 | 课程模块 | 学习产物 |
| --- | --- | --- | --- |
| 边界 | 这是不是投资建议？我应该怎么学？ | Module 0 | 学习边界和第一条编程任务|
...
- Mini Project 融合在课程内容里，帮助用户把课程进度转化为工程产物。
- 最终报告要求由第 9.4 课和 Python 报告脚本承接，要求用户从数据、策略、验证和限制四个方面完成总结。
...
| Prompt 只写“帮我实现” | 工具或学习者容易遗漏约束和边界 | 写成带背景、目标、约束、验收的工单 |
