...
lessons.push(
  ...
  makeLesson({
    ...
    concepts: ["best parameter", "transfer", "validation"],
    intuition: "如果参数只是在历史噪声上碰巧最优，换一段时间后它可能立刻失效。",
...
lessons.push(
  makeLesson({
    ...
    intuition: "动量策略假设强者可能继续强一段时间。但这只是市场假设，不是自然规律。",
  ...
  makeLesson({
    ...
    handExample: "双均线函数如果 fast_window >= slow_window，就应该抛出清晰错误，而不是生成一个含糊结果。",
    pythonCode: code.movingAverage,
    chart: "moving-average",
  ...
  makeLesson({
    ...
    handExample: "比较 buy and hold 和双均线时，两者应使用同一段价格数据、同一 signal lag 和同一成本设置。",
  ...
  makeLesson({
    ...
    intuition: "组合不是把股票名字放在一起，而是用权重决定每只资产对结果的贡献。",
  ...
  makeLesson({
    ...
    handExample: "如果组合收益略低于 SPY，但最大回撤明显更小，它可能仍然有学习讨论价值。",
    ...
    mistakes: ["组合只和现金比", "股票数量多就宣称分散", "不看回撤"],
...
lessons.push(
  ...
  makeLesson({
    ...
    handExample: "如果用当天涨跌后才知道的收盘价决定当天是否持仓，就等于提前知道了当天结局。",
    pythonCode: code.backtest,
    chart: "costs",
  ...
  makeLesson({
    ...
    intuition: "报告不是宣传材料，而是研究记录。它应该说清楚数据、方法、结果和限制。",
...
lessons.push(
  makeLesson({
    ...
    handExample: "只选择现在还存在的大公司做历史回测，会忽略过去失败或退市的公司，这就是幸存者偏差。",
  ...
  makeLesson({
    ...
    handExample: "一个策略在低利率牛市表现很好，不代表在高利率震荡市仍然有效。",
  ...
  makeLesson({
    ...
    handExample: "模拟日志写着买入信号出现，不代表真实账户能按理想价格成交。",
  ...
  makeLesson({
    ...
    handExample: "报告应包含：数据来源、复权列选择、策略假设、回测配置、成本、benchmark、参数扫描、样本外和风险声明。",
    pythonCode: code.report,
    chart: "strategy-comparison",
  ...
  makeLesson({
    ...
    handExample: "下一阶段可以选择：统计基础、时间序列、因子研究、组合优化、事件驱动回测或数据工程。",
