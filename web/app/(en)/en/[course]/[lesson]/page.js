...
export function generateStaticParams() {
  return getLessonStaticParams("en");
}

export async function generateMetadata({ params }) {
  const { course, lesson } = await params;
  const page = getLessonBySlug("en", course, lesson);
...
export default async function Page({ params }) {
  ...
  return <LessonDetailPage currentIndex={page.index} currentLessonIndex={page.lessonIndex} locale="en" />;
}
